...
type NodeOutputsWidget struct {
	*Table
	Control
	eventsCh chan ui.Event
}

func NewNodeOutputsWidget(eventsHub *EventsHub) *NodeOutputsWidget {
	nodes := &NodeOutputsWidget{
		Table:    NewTable(),
		eventsCh: eventsHub.Subscribe(),
	}

	nodes.Title = "Node Outputs"
...
func (nodes *NodeOutputsWidget) Draw(buf *ui.Buffer) {
	if nodes.Visible {
		nodes.Table.Draw(buf)
	}
}

func (nodes *NodeOutputsWidget) controller() {
...
func (nodes *NodeOutputsWidget) update(ports []orchestra.Port) {
	numPorts := len(ports)
	...
	ui.Render(nodes)
	log.Logger.Debugf("nodeOutputs.Table.Rows: %v, SelectedRow: %d", nodes.Rows, nodes.SelectedRow)
}
